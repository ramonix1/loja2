...
// S처 dispon홼el em desenvolvimento
...
// Resetar conex찾o do pool
router.post('/reset/:slug', devOnly, async (req, res) => {
  const { slug } = req.params;
  try {
    invalidatePool(slug);
    res.redirect('/_tenants?msg=Conex찾o+resetada+para+' + encodeURIComponent(slug));
  } catch (err) {
    res.redirect('/_tenants?erro=' + encodeURIComponent(err.message));
  }
});
