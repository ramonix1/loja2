...
| **Status** | `approved` · P4 `done` |
| **Escopo** | `apps/admin` (Platform Hub `/platform/*` + Admin lojista `/admin/*`) + `packages/ui` |
| **Referência visual** | Stellarsync — dashboard multi-loja (ver §0) |
| **Arquivo de referência** | [`docs/design/references/stellarsync-platform-dashboard.png`](../design/references/stellarsync-platform-dashboard.png) |
...
## 0. Referências visuais

Mockup enviado pelo time como inspiração para o **Platform Ops** (gestão de lojas + KPIs). **Não** é identidade Ata Labs — serve para extrair padrões de layout, densidade de informação e componentes.

![Stellarsync — dashboard Platform / Store (referência UI)](../design/references/stellarsync-platform-dashboard.png)

| Campo | Valor |
|-------|-------|
| **Arquivo** | `docs/design/references/stellarsync-platform-dashboard.png` |
| **Origem** | Mockup interno “Stellarsync” (referência de mercado SaaS multi-loja) |
| **Tela** | Visão **Store** — sidebar + header + faixa KPI + grid de lojas |
| **Usar como referência para** | §2 (sidebar/header), §5 (AppShell), §6 (dashboard + cards de loja) |
| **Não copiar literalmente** | Paleta verde Stellarsync, ratings fictícios, “Premium Access”, copy em inglês |

**Elementos mapeados na imagem:**

1. **Sidebar** — logo, busca global (`⌘K`), nav agrupada (MAIN MENU / FEATURES / OTHER), sub-itens expansíveis, perfil no rodapé.
2. **Header** — breadcrumbs, ações à direita (chat, notificações, customize, toggle tema).
3. **KPI strip** — card único com duas linhas de métricas (contagens + performance).
4. **Toolbar** — busca, filtro, toggle grid/lista, CTA primário “Add Store”.
5. **Store cards** — logo, nome, status pill, grid 2×2 de metadados, CTAs no footer.
6. **Paginação** — números + “Showing X to Y of Z entries”.

> Se novas referências forem adicionadas (ex.: detalhe de loja, dark mode), salvar em `docs/design/references/` com nome descritivo e listar nesta seção.
...
| **Toggle tema** | Sol/lua no **NavUser** (dropdown perfil) | Tema fora do header; sidebar só navegação + perfil |
...
| 7 | **NavUser** | `SidebarMenuButton` + dropdown com `data-ui-surface`; avatar iniciais; **tema claro/escuro no menu** | Tema + Sair | + Trocar loja, Ver vitrine |

**Collapse:** `collapsible="icon"` + `SidebarRail`; estado persistido em cookie (padrão shadcn).

**Comportamento mobile:** sidebar no Sheet do primitivo shadcn; header com `SidebarTrigger` (hamburger).
...
| `NavUser` | Avatar dropdown (rodapé sidebar) | `user-menu-trigger` |
| `CommandPalette` | Dialog `⌘K` (shadcn Command) | `command-palette` |
| `NotificationsBell` | Sino + dropdown de alertas | `notifications-*` |
...
| `GET` | `/api/v1/platform/merchants` | query: `q`, `status`, `page`, `limit` | P3 |
| `GET` | `/api/v1/platform/stores/:slug/metrics` | KPIs da loja | P2/P4 |
| `GET` | `/api/v1/platform/stores/:slug/billing` | Billing read-only merchant | P4 |
| `GET` | `/api/v1/platform/health` | Resumo saúde operacional | P4 |
| `GET` | `/api/v1/platform/reports` | Relatórios billing/onboarding | P4 |
...
| **P3** | `shell-p3` | Command palette; notificações; merchants list; filtros server-side | ⌘K funcional · **done** |
| **P4** | `shell-p4` | Store detail tabs; billing; health; analytics | Platform completo MVP+ · **done** |
...
- `⌘K` / `Ctrl+K` abre `command-palette` e navega para `/platform/merchants`.
- Lista merchants com busca server-side (`platform-merchants-*`).
- Filtros `status` + `plano` em `/platform/stores` refletem query string.
- Login platform redireciona para `/platform/dashboard`.

**E2E mínimo P4:**

- Detalhe loja → tabs Visão geral / Merchant / Billing / Ações.
- `/platform/health` e `/platform/reports` carregam KPIs.

---

## 12. Critérios de aceite (DoD global)

- [x] Platform abre em `/platform/dashboard` (não só lista).
- [x] Sidebar platform ≥ 4 itens úteis agrupados; admin reorganizado em ≥ 3 grupos.
- [x] Header desktop visível em **ambas** personas com breadcrumbs e toggle tema.
- [x] Lista de lojas platform em **grid de cards** com busca e paginação.
- [x] Faixa KPI unificada no dashboard platform (mín. 4 métricas master DB).
- [x] `LayoutAdmin` deprecated com alias para `AppShell` (sem quebrar imports).
- [ ] Mobile: touch 48px; toolbar empilha verticalmente.
- [x] `pnpm turbo typecheck` + `make test-api` + E2E smoke platform P3/P4 verdes.
...
| 1.1 | 2026-07-02 | §0 Referências visuais + asset `stellarsync-platform-dashboard.png` |
| 1.2 | 2026-07-02 | P0 concluído — tokens `--shell-*`, spec `approved`, início P1 |
| 1.3 | 2026-07-02 | P1 Platform — `AppShell`, dashboard KPI, lojas grid+toolbar, API stats/paginação |
| 1.4 | 2026-07-02 | P2 Admin — `AppShell` lojista, nav agrupada (4 grupos), header+breadcrumbs, `KpiStrip` dashboard, `LayoutAdmin` → alias `AppShell`, testid `kpi-strip` no admin |
| 1.5 | 2026-07-02 | P3 — `CommandPalette` (`cmdk` + `⌘K`) e `NotificationsBell` em platform + admin; `GET /platform/merchants`; página `/platform/merchants`; filtros server-side `status=active` + `plano` em lojas; login platform → `/platform/dashboard`; testids + E2E `platform.spec.ts` |
| 1.6 | 2026-07-02 | P4 — detalhe loja com tabs (Visão geral/Merchant/Billing/Ações) + sidebar KPIs; `GET stores/:slug/metrics|billing`, `/platform/health`, `/platform/reports`; nav ANALYTICS; lógica `attention` em saúde; KPI `trialsExpiring7d`; testids + E2E |
