 ...
 *
 * Exemplos:
 *   node scripts/generate-invoices.js           (usa m√™s atual)
...
async function generateMonthlyInvoices() {
  try {
    ...
    console.log(`\nßæ Gerando invoices para ${monthYear}...\n`);
    ...
    for (const row of tenants.rows) {
      try {
        ...
        // Verificar se j√° existe invoice para esse m√™s
        ...
        if (existing.rows.length > 0) {
          console.log(`  Pulando ${tenantId} (invoice j√° existe)`);
          continue;
        }
    ...
    // Relat√≥rio final
    console.log(`\n${'Ä'.repeat(60)}`);
    console.log(`[RESUMO] DA GERA√√ÉO DE INVOICES (${monthYear})`);
    console.log(`${'Ä'.repeat(60)}`);
    ...
    if (Object.keys(byType).length > 0) {
      console.log('µ Receita por Tipo de Billing:');
