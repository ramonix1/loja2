#!/usr/bin/env bash
# Valida storefront como no CI: /health, SSR da home e rewrite /api/v1 de dentro do container.
set -euo pipefail
...
echo "[verify-storefront] rewrite /api/v1/public/store de dentro do container..."
$COMPOSE exec -T storefront node -e "
  fetch('http://127.0.0.1:3000/api/v1/public/store', {
    headers: { 'X-Tenant-Slug': 'loja' },
  })
    .then((r) => {
      console.log('in-container rewrite', r.status);
      process.exit(r.ok ? 0 : 1);
    })
    .catch((e) => {
      console.error(e);
