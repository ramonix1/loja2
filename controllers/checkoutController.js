...
async function getLojaInfo(db) {
  ...
  return { nome: cfg.loja_nome || 'LojÃ£o', email: cfg.loja_email || '' };
}
...
exports.processarCheckout = async (req, res) => {
  ...
  if (errosValidacao.length > 0) {
    ...
    console.warn('ValidaÃ§Ã£o de checkout falhou:', errosValidacao);
  ...
  console.log(`[Checkout] POST recebido  mÃ©todo: ${metodo_pagamento}, usuÃ¡rio: ${usuarioId}`);

  // Verificar agenda antes de iniciar transaÃ§Ã£o
  ...
  try {
    ...
    // Registrar agendamento (se mÃ³dulo ativo)
    ...
    // CartÃ£o com 3DS: redireciona para pÃ¡gina de autenticaÃ§Ã£o do Stripe
