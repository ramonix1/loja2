...
exports.disponibilidadeMes = async (req, res) => {
  try {
    const mes = req.query.mes || new Date().toISOString().slice(0, 7);
    const [ano, mesNum] = mes.split('-').map(Number);
    if (!ano || !mesNum) return res.status(400).json({ ok: false, erro: 'mes invÃ¡lido' });
    ...
    console.error('Erro disponibilidade mÃªs:', err);
...
exports.verificarDisponibilidade = async (req, res) => {
  try {
    ...
    if (!data || !/^\d{4}-\d{2}-\d{2}$/.test(data)) {
      return res.status(400).json({ ok: false, erro: 'Data invÃ¡lida' });
    }
