...
export async function getPublicProductReviews(
  ...
  return reply.send({
    data: reviews,
    meta: { page: parsed.data.page, perPage: parsed.data.limit, total },
  });
}

export async function getAdminReviews(request: FastifyRequest, reply: FastifyReply): Promise<void> {
  const parsed = listReviewsQuerySchema.safeParse(request.query);
  if (!parsed.success) {
    return reply.code(400).send({
      error: 'Parâmetros inválidos.',
...
export async function patchReviewStatus(
  ...
  const data = await updateReviewStatus(requireStoreScope(request), reviewId, parsed.data.status);
  if (!data) {
