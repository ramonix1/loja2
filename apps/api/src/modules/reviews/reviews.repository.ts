...
export async function findRatingSummariesByProductIds(
  scope: StoreScope,
  productIds: number[],
...
export async function findBuyerEligibility(
  scope: StoreScope,
  buyerId: number,
  productId: number,
): Promise<boolean> {
  const res = await scope.pool.query(
    `SELECT 1
     ...
     WHERE o.store_id = $1 AND o.buyer_id = $2 AND o.status = 'delivered' AND oi.product_id = $3
     LIMIT 1`,
...
export async function productExistsForReview(scope: StoreScope, productId: number): Promise<boolean> {
  const res = await scope.pool.query(
...
export async function insertProductReview(
  ...
  try {
    ...
    const err = e as { code?: string };
    if (err.code === '23505') return { code: '23505' };
    throw e;
  }
}
...
export async function findAdminReviews(
  ...
  `;
  const countSql = `SELECT COUNT(*)::int AS total FROM product_reviews r WHERE r.store_id = $1${statusFilter}`;
...
export async function updateReviewStatusRecord(
  scope: StoreScope,
  reviewId: number,
