import type { PedidoDetalhe, UpdatePedidoStatusInput } from '@lojao/types/pedidos';
...
/** Estatísticas brutas do dashboard — queries SQL tenant-scoped. */
export async function fetchDashboardStatsRows(db: pg.Pool) {
  ...
  return { hoje, pendentes, receita, produtos, categorias, banners, totalPedidos, receitaTotal, recentes };
}

/** Lista paginada de pedidos (read-only) com dados do cliente — Drizzle. */
export async function fetchPedidosList(
  db: TenantDatabase,
...
export async function fetchPedidoById(db: pg.Pool, id: number): Promise<PedidoDetalhe | null> {
...
export async function updatePedidoStatusRow(
  db: pg.Pool,
  id: number,
