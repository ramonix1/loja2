...
export async function getPermissoes(request: FastifyRequest, reply: FastifyReply): Promise<void> {
  const data = await listAdmins(requireStoreScope(request));
  return reply.send({ data });
}

export async function createPermissao(request: FastifyRequest, reply: FastifyReply): Promise<void> {
