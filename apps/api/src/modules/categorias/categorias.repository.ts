...
export async function findAllCategorias(db: pg.Pool): Promise<CategoriaListItem[]> {
...
export async function findCategoriaById(db: pg.Pool, id: number): Promise<CategoriaDetail | null> {
...
export async function categoriaExists(db: pg.Pool, id: number): Promise<boolean> {
...
export async function deleteCategoriaRecord(db: pg.Pool, id: number): Promise<void> {
  await db.query('UPDATE produtos SET categoria_id = NULL WHERE categoria_id = $1', [id]);
