...
export async function getProdutos(request: FastifyRequest, reply: FastifyReply): Promise<void> {
  const data = await listProdutos(requireStoreScope(request));
  return reply.send({ data });
}

export async function getProdutoById(request: FastifyRequest, reply: FastifyReply): Promise<void> {
...
export async function updateEstoque(request: FastifyRequest, reply: FastifyReply): Promise<void> {
  const id = Number((request.params as { id: string }).id);
  if (!Number.isInteger(id) || id < 1) {
    return reply.code(400).send({ error: 'ID inválido.', code: 'VALIDATION_ERROR' });
  }
...
export async function deleteProdutoImagemHandler(
  ...
  const ok = await deleteProdutoImagem(
    requireStoreScope(request),
    request.server.imageStorage,
    imagemId,
  );
