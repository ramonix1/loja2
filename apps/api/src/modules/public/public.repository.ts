...
export async function findStoreConfigs(db: pg.Pool): Promise<{
...
export async function findPublicCategoriesWithProducts(db: pg.Pool): Promise<PublicCategory[]> {
  const res = await db
    .query(
      `
...
export async function findPublicProductsWithoutCategory(db: pg.Pool): Promise<PublicProduct[]> {
...
export async function findAllPublicProducts(db: TenantDatabase): Promise<PublicProduct[]> {
  const rows = await db
    .select({
      id: produtos.id,
      nome: produtos.nome,
...
export async function findPublicBanners(db: pg.Pool): Promise<PublicBanner[]> {
  const res = await db
    .query(
