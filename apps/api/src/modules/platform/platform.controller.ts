...
export async function postPlatformLogin(
  ...
  return reply.send({ data: { email: creds.email, role: 'platform_admin' } });
}
...
export async function getPlatformDashboardStatsHandler(
...
export async function getPlatformDashboardChartsHandler(
  request: FastifyRequest,
  reply: FastifyReply,
): Promise<void> {
...
export async function postPlatformStore(
  ...
  if (!result.ok) {
    return reply.code(409).send({ error: 'Já existe uma loja com esse slug.', code: 'SLUG_EXISTS' });
  }
...
export async function getPlatformStoreMetrics(
  request: FastifyRequest,
  reply: FastifyReply,
): Promise<void> {
  const slug = (request.params as { slug: string }).slug;
...
export async function getPlatformStoreBilling(
  request: FastifyRequest,
  reply: FastifyReply,
): Promise<void> {
  const slug = (request.params as { slug: string }).slug;
...
export async function patchPlatformStore(
  request: FastifyRequest,
  reply: FastifyReply,
): Promise<void> {
  const slug = (request.params as { slug: string }).slug;
